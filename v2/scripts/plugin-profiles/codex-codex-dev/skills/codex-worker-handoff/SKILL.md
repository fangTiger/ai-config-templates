...
在 `codex-codex-dev` 模式下，Architecture Codex 完成分析、设计与任务拆解后，通过本技能把实现工作交给 Implementation Codex。
...
- 除本 profile 默认 Review Codex 外，前端、专项审查或全局一致性 subagent / specialist 只能在用户明确要求或任务包批准时接入
- 三角色入口口径见项目根 `AGENTS.md`；复杂阶段推进见 `.codex/workflow.md`
...
多 Implementation Codex 完成后，无论是否存在 subagent / specialist，都必须先执行 integration checkpoint：
...
额外的前端、专项审查或全局一致性 subagent / specialist sync 仅在任务包明确批准时执行；这不影响默认 Review Codex gate。
...
如需获批 subagent / specialist 参与，Architecture Codex 应交付：
- API 契约摘要
- 前端任务清单
- 技术栈约束
- UI/UX 约束
...
如果 subagent / specialist 发现契约无法满足场景需求，Architecture Codex 必须做裁决：
...
3. 使用 `codex-review` 整理 Review Input 和审查清单，然后显式启动 `.codex/agents/review-codex.toml` 定义的 `review-codex` custom agent；若 subagent 不可用，记录替代 review 来源
...
混合任务:
ANALYZE → DESIGN → slice HANDOFF → one/many Implementation Codex 实现 → Integration/Sync Gate → 获批 subagent / specialist → Review Codex

纯前端:
可跳过 Implementation Codex，按已批准任务包交给对应 subagent / specialist，仍需 Review Codex 汇总
```
