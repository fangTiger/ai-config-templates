...
# Python profile 写入工具执行前，用 graphify 补充 Python 影响面上下文。
...
def is_python_target(target_file: str) -> bool:
    normalized = target_file.replace("\\", "/")
    return (
        normalized.endswith(".py")
        or normalized in PYTHON_METADATA
...
if not target_file or not is_python_target(target_file):
    raise SystemExit(0)

graph_file = os.environ.get("GRAPHIFY_GRAPH_PATH") or os.path.join(cwd, "graphify-out", "graph.json")
report_file = os.path.join(cwd, "graphify-out", "GRAPH_REPORT.md")

structure_query = f"{target_file} architecture dependencies"
impact_query = f"{target_file} impact callers tests dependencies"
lines = [
    f"graphify: Before modifying `{target_file}`, inspect Python structure and impact first.",
    "Recommended graphify checks:",
    f'- `graphify query "{structure_query}"`',
    f'- `graphify query "{impact_query}"`',
    "Summarize graphify impact, callers, dependencies, affected tests, and Python validation commands before you continue.",
]
