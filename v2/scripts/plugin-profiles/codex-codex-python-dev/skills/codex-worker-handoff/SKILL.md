...
description: Orchestrates structured context handoff from Architecture Codex to Implementation Codex for Python implementation. Use when an approved OpenSpec proposal or approved task package needs implementation in codex-codex-python-dev mode. Handles context packaging, Implementation Codex session startup, file-scope guardrails, progress checkpoints, evidence collection, optional Codex review sync, and review handback.
---
...
在 `codex-codex-python-dev` 模式下，Architecture Codex 完成分析、设计与任务拆解后，通过本技能把 Python 实现工作交给 Implementation Codex。
...
- 三角色入口、Python-first 六阶段流水线和 gate 口径均以项目根 `AGENTS.md` 为准
...
4. PythonDetection 已运行，且虚拟环境、依赖文件、测试入口和推荐验证命令已记录
...
- `PythonDetection`: 检测命令、classification、dependency_files、virtualenv、layout、recommended_validation_commands、init_allowed
...
```markdown
# codex-codex-python-dev Workflow State
## Mode: codex-codex-python-dev
## PythonProjectClassification: [empty-python-project|existing-python-project]
## RecommendedValidationCommands: [commands]
