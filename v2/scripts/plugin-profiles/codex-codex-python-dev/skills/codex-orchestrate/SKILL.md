...
description: Orchestrates codex-codex-python-dev workflow. Use when the Architecture Codex needs to analyze a Python coding request, decide whether OpenSpec is required, split tasks, prepare implementation handoff, and manage Implementation Codex execution boundaries.
---
...
在 `codex-codex-python-dev` 模式下，Architecture Codex 使用本技能完成：
- 任务分级（小 / 中 / 大）
- Python 项目检测结果读取与约束固化
- 是否需要 OpenSpec 的判断
...
这是 **Architecture Codex** 的治理技能，不是直接实现技能。三角色入口、Python-first 六阶段流水线和 gate 口径均以项目根 `AGENTS.md` 为准。

## 触发条件

- 项目根 `AGENTS.md` 明确当前入口契约为 `codex-codex-python-dev`
- 用户提出中等及以上复杂度的开发需求
- 关键词：实现、设计、拆解、编排、开始开发、开始实现、handoff、交接、审查、Clarify Gate、Graphify、影响面、Python 检测

## 步骤 1：读取主控规则

开始前必须读取：
1. 项目根 `AGENTS.md`
2. 运行或复用本轮 `.codex/tools/detect-python-project.sh` 输出，记录 `classification`、`dependency_files`、`virtualenv`、`layout` 和 `recommended_validation_commands`
3. 项目根 `openspec/config.yaml`、`openspec/changes/`、`openspec/specs/`（如任务涉及变更提案）
4. 按 `AGENTS.md` 上下文优先级读取 `CODE_WIKI.md`、`docs/guide/*`、`docs/domain/*`、`docs/reference/*`、Graphify context、相关源码和测试；不存在时记录缺失与替代依据
5. 对 Python 架构、依赖、影响面、跨模块修改、非平凡搜索或 Python 代码修改，必须先获取 Graphify context：有 `graphify-out/graph.json` 时查询结构 / 影响面；无法查询时读取 `graphify-out/GRAPH_REPORT.md`；仍不可用时记录 `Graphify: unavailable` 及原因

## 步骤 1.5：Clarify Gate

进入 DESIGN 或 IMPLEMENT 前，Architecture Codex 必须确认：
- PythonDetection 是否已运行并可用于本轮
- Acceptance criteria 是否明确
- Out-of-scope 是否明确
- 是否需要 OpenSpec；空项目 scaffold 必须需要，跳过时是否有理由
- 每个中 / 大任务的 Executor 是否明确到 Architecture Codex、Implementation Codex、Review Codex 或人工负责人
- Validation 和 stop conditions 是否可执行，并且优先来自 `recommended_validation_commands`
...
- `PythonDetection` 必须记录检测命令和关键结果；空项目初始化必须绑定已批准 OpenSpec。
