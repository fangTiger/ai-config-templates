---
name: codex-review
description: Reviews Implementation Codex implementation results in codex-codex-python-dev mode. Use when Review Codex needs to validate Python detection, design alignment, file scope, test evidence, and decide whether to accept, request fixes, or downgrade.
---
...
在 `codex-codex-python-dev` 模式下，本技能用于 Review Codex 对 Implementation Codex 的结果进行治理性审查。
重点不是“帮忙写一点代码”，而是判断：
- 是否具备本轮 PythonDetection，并沿用检测到的虚拟环境、依赖文件、测试入口和推荐验证命令
...
这是 **Review Codex** 的审查技能。三角色入口、Python-first 六阶段流水线和 gate 口径均以项目根 `AGENTS.md` 为准。
...
### 2. Python-first 约束
必须检查：
- 是否先运行 `.codex/tools/detect-python-project.sh`
- 空项目是否在 OpenSpec 提案批准前避免 scaffold
- 是否沿用现有虚拟环境、依赖文件、测试目录和工具链
- 是否默认引入未声明框架、包管理器、lint/type 工具、数据库、Sentry 或 CI
- RED/GREEN 是否优先使用 `recommended_test_command` 或 `recommended_validation_commands`
...
### 7. 多 Implementation Codex 两阶段审查
