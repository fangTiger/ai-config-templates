...
description: Orchestrates codex-codex-claude-flow-dev workflow. Use when the Architecture Codex needs to analyze a coding request, decide whether OpenSpec is required, split tasks, prepare implementation handoff, and manage Implementation Codex execution boundaries.
---
...
在 `codex-codex-claude-flow-dev` 模式下，Architecture Codex 使用本技能完成：
- 任务分级（小 / 中 / 大）
- 是否需要 OpenSpec 的判断
...
这是 **Architecture Codex** 的治理技能，不是直接实现技能。三角色入口、六阶段流水线和 gate 口径均以项目根 `AGENTS.md` 为准。

## 触发条件

- 项目根 `AGENTS.md` 明确当前入口契约为 `codex-codex-claude-flow-dev`
- 用户提出中等及以上复杂度的开发需求
