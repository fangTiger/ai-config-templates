# codex-codex-claude-flow-gpt56-sol-dev 项目指令
...
`codex-codex-claude-flow-gpt56-sol-dev` 保留旧 `codex-dev/CLAUDE.md` 的核心体验：架构者先想清楚，再把实现交给执行者，最后由独立 Codex 审查和证据验证收口。

本 profile 的模型路由是模板级运行时约定，不作为 Codex 平台通用规则：

- Architecture Codex / 最外层主线程：`.codex/config.toml`，`gpt-5.6-sol` + `xhigh`。
- Implementation Codex / coding worker：`.codex/agents/worker-codex.toml`，`gpt-5.5` + `xhigh`。
...
1. 运行 `bash .codex/tools/runtime-verification-summary.sh` 消费 PostToolUse tracker 摘要，记录 tracker session id、编辑文件和建议验证命令；如未找到缓存、摘要为空、session id 不匹配或日志缺失，则记录 `PostToolUse tracker: degraded` 及原因，并继续使用 `git diff`、测试输出和 Review evidence 完成验证。
2. 对照 tracker 摘要、OpenSpec tasks、Review Codex 要求和实际 diff，确定 fresh verification commands；执行必要验证命令，并说明 tracker 建议命令中哪些已执行、哪些未执行及原因。
3. 检查 `git status --short`、`git diff --name-only` 和文件范围，确认未混入未声明文件、运行态文件或无关改动。
4. 检查 OpenSpec tasks 是否全部完成；若 Review Codex 是 gate，确认 Review Decision 为 `PASS` 后才允许继续收口。
5. 需要归档时执行 OpenSpec archive，并确认 delta 已进入 `openspec/specs/`。
6. 更新 `.codex/session-state.md` 的 `Current Stage`、`LastVerificationResult`、`ReviewDecision`、`CompletedTasks`、`PendingTasks` 和下一步提示；如归档 session-state，先写入 `.codex/session-state.archive/YYYYMMDDHHMMSS-<change-id>.md`，再从 `.codex/session-state.template.md` 重置运行态文件。
7. 最终回复列出读取依据、PostToolUse tracker 状态、修改文件、实际验证命令与结果、未验证项、阻塞项和剩余风险。
...
- `Mode: codex-codex-claude-flow-gpt56-sol-dev`
- `ActiveTaskStatus`
- `Current Stage`
- `CurrentTask`
- `Executor`
