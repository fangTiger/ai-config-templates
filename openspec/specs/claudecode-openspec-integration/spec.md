...
### Requirement: 实现前规范检查
Claude Code MUST 在开始任何非平凡实现任务之前检查现有的 OpenSpec 规范。
...
### Requirement: 自动提案触发检测
Claude Code MUST 根据定义的触发器检测何时需要创建 OpenSpec 提案。
...
### Requirement: OpenSpec 命令集成
Claude Code MUST 能够访问 OpenSpec 斜杠命令进行工作流管理。
...
### Requirement: 规范-实现一致性检查
Claude Code MUST 验证实现是否与对应的规范匹配。
